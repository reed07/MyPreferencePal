...
public abstract class AutoScrollHelper implements OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long j) {
            ...
            if (j2 < 0 || j < j2) {
                return AutoScrollHelper.constrain(((float) (j - this.mStartTime)) / ((float) this.mRampUpDuration), (float) BitmapDescriptorFactory.HUE_RED, 1.0f) * 0.5f;
            }
            long j3 = j - j2;
            float f = this.mStopValue;
            return (1.0f - f) + (f * AutoScrollHelper.constrain(((float) j3) / ((float) this.mEffectiveRampDown), (float) BitmapDescriptorFactory.HUE_RED, 1.0f));
        }
