                ...
                try {
                    if (bundle.containsKey("text")) {
                        if (bundle.containsKey("time")) {
                            Person person = bundle.containsKey(KEY_PERSON) ? Person.fromBundle(bundle.getBundle(KEY_PERSON)) : (!bundle.containsKey(KEY_NOTIFICATION_PERSON) || VERSION.SDK_INT < 28) ? bundle.containsKey(KEY_SENDER) ? new android.support.v4.app.Person.Builder().setName(bundle.getCharSequence(KEY_SENDER)).build() : null : Person.fromAndroidPerson((Person) bundle.getParcelable(KEY_NOTIFICATION_PERSON));
