...
public class MaterialShapeDrawable extends Drawable implements TintAwareDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (i > 0 && this.shadowEnabled) {
            this.paint.setShadowLayer((float) this.shadowRadius, BitmapDescriptorFactory.HUE_RED, (float) i, this.shadowColor);
        }
        if (this.shapedViewModel != null) {
            getPath(canvas.getWidth(), canvas.getHeight(), this.path);
            canvas.drawPath(this.path, this.paint);
        } else {
            canvas.drawRect(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, (float) canvas.getWidth(), (float) canvas.getHeight(), this.paint);
        }
    ...
    private void getCoordinatesOfCorner(int i, int i2, int i3, PointF pointF2) {
        switch (i) {
            case 1:
                pointF2.set((float) i2, BitmapDescriptorFactory.HUE_RED);
                return;
            case 2:
                pointF2.set((float) i2, (float) i3);
                return;
            case 3:
                pointF2.set(BitmapDescriptorFactory.HUE_RED, (float) i3);
                return;
            default:
    ...
    private float angleOfCorner(int i, int i2, int i3) {
        ...
        return atan2 < BitmapDescriptorFactory.HUE_RED ? (float) (((double) atan2) + 6.283185307179586d) : atan2;
    }
