...
public class MoPubNativeAppInstallAdMapper extends NativeAppInstallAdMapper {
    ...
    public MoPubNativeAppInstallAdMapper(StaticNativeAd staticNativeAd, HashMap<String, Drawable> hashMap, int i, int i2) {
        ...
        if (hashMap != null) {
            setIcon(new MoPubNativeMappedImage((Drawable) hashMap.get(DownloadDrawablesAsync.KEY_ICON), this.mMopubNativeAdData.getIconImageUrl(), 1.0d));
            ArrayList arrayList = new ArrayList();
            arrayList.add(new MoPubNativeMappedImage((Drawable) hashMap.get(DownloadDrawablesAsync.KEY_IMAGE), this.mMopubNativeAdData.getMainImageUrl(), 1.0d));
            setImages(arrayList);
        } else {
            setIcon(new MoPubNativeMappedImage(null, this.mMopubNativeAdData.getIconImageUrl(), 1.0d));
            ArrayList arrayList2 = new ArrayList();
            arrayList2.add(new MoPubNativeMappedImage(null, this.mMopubNativeAdData.getMainImageUrl(), 1.0d));
                ...
                if (context != null) {
                    ...
                    this.privacyInformationIconImageView.setOnClickListener(new OnClickListener() {
                        public void onClick(View view) {
                            new Builder().withSupportedUrlActions(UrlAction.IGNORE_ABOUT_SCHEME, UrlAction.OPEN_NATIVE_BROWSER, UrlAction.OPEN_IN_APP_BROWSER, UrlAction.HANDLE_SHARE_TWEET, UrlAction.FOLLOW_DEEP_LINK_WITH_FALLBACK, UrlAction.FOLLOW_DEEP_LINK).build().handleUrl(context, privacyInformationIconClickThroughUrl);
                        }
                    });
                    this.privacyInformationIconImageView.setVisibility(0);
                    ((ViewGroup) childAt).addView(this.privacyInformationIconImageView);
                    int i = (int) (((double) (((float) this.mPrivacyIconSize) * context.getResources().getDisplayMetrics().density)) + 0.5d);
