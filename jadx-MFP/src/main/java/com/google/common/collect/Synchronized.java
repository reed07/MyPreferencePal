...
final class Synchronized {
    ...
    private static final class SynchronizedTable<R, C, V> extends SynchronizedObject implements Table<R, C, V> {
        ...
        public Map<R, Map<C, V>> rowMap() {
            Map<R, Map<C, V>> map;
            synchronized (this.mutex) {
                map = Synchronized.map(Maps.transformValues(delegate().rowMap(), (Function<? super V1, V2>) new Function<Map<C, V>, Map<C, V>>() {
        ...
        public Map<C, Map<R, V>> columnMap() {
            Map<C, Map<R, V>> map;
            synchronized (this.mutex) {
                map = Synchronized.map(Maps.transformValues(delegate().columnMap(), (Function<? super V1, V2>) new Function<Map<R, V>, Map<R, V>>() {
    ...
    /* access modifiers changed from: private */
    @GwtIncompatible
    public static <K, V> Entry<K, V> nullableSynchronizedEntry(@NullableDecl Entry<K, V> entry, @NullableDecl Object obj) {
        if (entry == null) {
            return null;
        }
