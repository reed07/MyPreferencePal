package com.myfitnesspal.feature.search.repository;

import com.myfitnesspal.shared.model.v1.DiaryEntryCellModel;
import com.uacf.core.util.Strings;
import java.util.Comparator;
import kotlin.Metadata;
import kotlin.TypeCastException;
import kotlin.comparisons.ComparisonsKt;
import kotlin.jvm.internal.Intrinsics;

@Metadata(bv = {1, 0, 3}, d1 = {"\u0000\u001e\n\u0000\n\u0002\u0010\b\n\u0002\b\u0006\n\u0002\b\u0006\n\u0002\b\u0006\n\u0002\b\u0006\n\u0002\b\u0006\n\u0002\b\u0007\u0010\u0000\u001a\u00020\u0001\"\u0004\b\u0000\u0010\u00022\u000e\u0010\u0003\u001a\n \u0004*\u0004\u0018\u0001H\u0002H\u00022\u000e\u0010\u0005\u001a\n \u0004*\u0004\u0018\u0001H\u0002H\u0002H\n¢\u0006\u0004\b\u0006\u0010\u0007¨\u0006\b"}, d2 = {"<anonymous>", "", "T", "a", "kotlin.jvm.PlatformType", "b", "compare", "(Ljava/lang/Object;Ljava/lang/Object;)I", "kotlin/comparisons/ComparisonsKt__ComparisonsKt$compareByDescending$1"}, k = 3, mv = {1, 1, 13})
/* compiled from: Comparisons.kt */
public final class LocalFoodSearchRepository$sortResultsList$$inlined$sortByDescending$1<T> implements Comparator<T> {
    public final int compare(T t, T t2) {
        String strings = Strings.toString(((DiaryEntryCellModel) t2).summaryLine1());
        Intrinsics.checkExpressionValueIsNotNull(strings, "Strings.toString(it.summaryLine1())");
        if (strings != null) {
            String lowerCase = strings.toLowerCase();
            Intrinsics.checkExpressionValueIsNotNull(lowerCase, "(this as java.lang.String).toLowerCase()");
            Comparable comparable = lowerCase;
            String strings2 = Strings.toString(((DiaryEntryCellModel) t).summaryLine1());
            Intrinsics.checkExpressionValueIsNotNull(strings2, "Strings.toString(it.summaryLine1())");
            if (strings2 != null) {
                String lowerCase2 = strings2.toLowerCase();
                Intrinsics.checkExpressionValueIsNotNull(lowerCase2, "(this as java.lang.String).toLowerCase()");
                return ComparisonsKt.compareValues(comparable, lowerCase2);
            }
            throw new TypeCastException("null cannot be cast to non-null type java.lang.String");
        }
        throw new TypeCastException("null cannot be cast to non-null type java.lang.String");
    }
}
